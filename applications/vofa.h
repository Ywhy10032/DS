...
/**
  ...
  *        恒为 0(哪怕球在动)，不代表视觉丢了
  *   ch8  当前任务号，1~7 对应任务一~任务七(TASK_ID+1) —— 跟下面 N 指令
  ...
  *   B<cms2>            刹车曲线的减速度，例如 B10.5 —— 速度上限
  *                       = sqrt(2 x B x 剩余距离)，"提前减速"就靠它。
  *                       调小=更早减速更稳更慢，调大=更晚减速更快但可能
  *                       冲过目标，0=关闭。任务三防过冲的主旋钮，
  *                       见 ball.h 的 BALL_POS_BRAKE_CMS2
  *   V<cms>             速度指令上限，例如 V12 —— 见 ball.h 的
  *                       BALL_VEL_LIMIT_CMS。任务三大部分时间受 B 的刹车
  *                       曲线约束而不是受它约束，想提速优先调 B
  *   F<us_per_rpms>     加速度前馈增益，例如 F7.0 —— 治"起步球被甩到后面"。
  *                       看起步那一下位置的峰值：还往后滑(x 增大)就加大，
  *                       反而往前冲就减小。它不属于 Ball_Tune，所以 R 指令
  *                       和按 KEY2 都冲不掉它，运行前标好能留到下一趟起步，
  *                       见 ball.h 的 BALL_FF_US_PER_RPMS
  *   N<1~7>             切换到指定任务，发几就是任务几(1=任务一...6=任务六，
  *                       没有任务 0)，等价于按 KEY1 循环切到该任务；运行中
  *                       发送会被忽略，规则与物理按键一致(见 task.h)。
  *                       N7 = 隐藏的倒车任务：直着往后倒 0.5m 自己停(参数见
  *                       task.h 的 TASK7_*)。它不在短按 KEY1 的循环里，
  *                       只能靠 N7 或长按 KEY1 进入；N7 只是切过去，
  *                       跟别的任务一样还要发 G 才开动
